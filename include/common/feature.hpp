...
{	
	...
	first_cam_pose.linear() = math_lib::quaternionToRotation(cam_states.find(first_cam_id)->second.orientation).transpose();
	first_cam_pose.translation() = cam_states.find(first_cam_id)->second.position;

	// 构造的是T_c_w,表示向量从c系到w系的变换矩阵
	Eigen::Isometry3d last_cam_pose;
	last_cam_pose.linear() = math_lib::quaternionToRotation(cam_states.find(last_cam_id)->second.orientation).transpose();
...
bool Feature::initializePosition(const CamStateServer& cam_states) 
{
	...
	for (auto& m : observations) {
		...
		cam0_pose.linear() = math_lib::quaternionToRotation(cam_state_iter->second.orientation).transpose(); // cam0 -> world
		cam0_pose.translation() = cam_state_iter->second.position;

		Eigen::Isometry3d cam1_pose;
		cam1_pose = cam0_pose * CAMState::T_cam0_cam1.inverse(); // cam1 -> world
	...
	// 全部cam pos以第一个cam pos为参考
	for (auto& pose : cam_poses) {
		pose = pose.inverse() * T_c0_w; // cam0 -> cami
	}
